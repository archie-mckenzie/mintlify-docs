...
```mermaid
  flowchart LR
    ...
    %% ^ Diese Subgraphen sind identisch, abgesehen von den Verknüpfungen zu ihnen:

    %% Verknüpfung *zu* subgraph1: Die Richtung von subgraph1 bleibt erhalten
    outside --> subgraph1
    %% Verknüpfung *innerhalb* von subgraph2:
    %% subgraph2 erbt die Richtung des obersten Diagramms (LR)
    outside ---> top2
```
````
...
````mdx
```mermaid
// Hier steht Ihr Mermaid-Diagrammcode
```
````
