...
<div className="pt-10 pb-24 px-4 px-4 max-w-3xl mx-auto prose prose-gray">
  ...
  <label className="text-lg prose prose-gray dark:prose-invert">ドキュメントの外観をカスタマイズ<br /><br />docs.json の [theme](ja/settings#param-theme) に、以下のいずれかのテーマを設定します。</label>
  ...
  <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-8 not-prose">
    ...
    <ThemeCard title="Maple" value="maple" description="AIやSaaSプロダクトに最適な、モダンでクリーンなデザイン。" href="https://maple.mintlify.app" />

    <ThemeCard title="Palm" value="palm" description="エンタープライズ向けドキュメントのための深いカスタマイズ性を備えた、洗練されたフィンテック向けテーマ。" href="https://palm.mintlify.app" />

    <ThemeCard title="Willow" value="willow" description="余計な要素をそぎ落とし、執筆に集中できる必須要素のみのデザイン。" href="https://willow.mintlify.app" />

    <ThemeCard title="Linden" value="linden" description="等幅フォントが映える、80年代ハッカーの雰囲気を持つレトロなターミナル風デザイン。" href="https://linden.mintlify.app" />

    <ThemeCard title="Almond" value="almond" description="カードベースの構成とミニマルなデザインで、直感的なナビゲーションを実現。" href="https://almond.mintlify.app" />

    <ThemeCard title="Aspen" value="aspen" description="複雑なナビゲーションとカスタムコンポーネントに最適化されたモダンなドキュメントテーマ。" href="https://aspen.mintlify.app" />
  </div>
</div>
