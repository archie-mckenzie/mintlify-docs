...
ドキュメントにカスタマーサポート用ウィジェットを組み込みます。`docs.json` に `integrations` フィールドを追加し、各アプリの ID を指定してください。
